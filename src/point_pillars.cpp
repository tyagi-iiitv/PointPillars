...
// Calculates the IOU between two bounding boxes.
float iou3D(const BoundingBox3D& box1,
          const BoundingBox3D& box2)
{
    const auto& box_as_vector = boundingBox3DToTopDown(box1);
    const auto& box_as_vector_2 = boundingBox3DToTopDown(box2);
    ...
    float topZ = min(box1.height/2 + box1.z, box2.height/2 + box2.z);
    float bottomZ = max( - box1.height/2 + box1.z, - box2.height/2 + box2.z);

    float volume_overlap = polygonArea(clipped_vector) * (topZ - bottomZ);

    return volume_overlap / (volume_poly1 + volume_poly2 - volume_overlap);
}
    ...
    for (const auto& labelBox: labelBoxes) //For every label box which is a 3d bounding box
    {
            ...
            // If none of the anchors passed the threshold, then we place the best anchor details for that object. 
        {
            ...
            if (printTime)
            {
                ...
                py::print("\nBest IOU of anchor " + std::to_string(bestAnchorId) + " was "+std::to_string(maxIou)+" Adding the best location regardless of threshold");
