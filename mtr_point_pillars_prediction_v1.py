...
from mtr_processors_v1 import CustomDataGenerator, AnalyseCustomDataGenerator
from inference_utils_mtr_v1 import generate_bboxes_from_pred
from inference_utils_mtr_v1 import focal_loss_checker, rotational_nms, generate_bboxes_from_pred_and_np_array, convert_boxes_to_list
...
if __name__ == "__main__":
    ...
    real_gen = AnalyseCustomDataGenerator(batch_size=params.batch_size,  root_dir=DATA_ROOT, 
                    point_cloud_statistics_path=PC_STATISTICS_PATH,
                    random_select=False,
                    npoints=8000, split='real', classes=list(params.classes_map.keys()))
    inference_duration = []
    sample_index = 0 # has to be controlled manually to ensure that the sequence number is continuous
    print("total number of batch: ", len(real_gen))
    for batch_idx in range(0, len(real_gen)):
        [pillars, voxels], [pts_input] = real_gen[batch_idx]
        ...
        print("batch_idx ", batch_idx, " has ", loop_range, "batch sample", " with occupancy sum: ", np.sum(occupancy))
        for i in range(loop_range):
            set_box, predicted_boxes3d = generate_bboxes_from_pred_and_np_array(occupancy[i], position[i], size[i], angle[i], 
                                                            heading[i],
                                                            classification[i], params.anchor_dims, occ_threshold=occ_threshold)
            ...
            if len(set_box) > 0:
                ...
                nms_boxes = rotational_nms([set_box], [confidence], occ_threshold=occ_threshold, nms_iou_thr=nms_iou_thr)
                ...
                print("sample_: ", batch_idx * params.batch_size + i, " has ", predicted_boxes3d_.shape, "predictions")
            ...
            Converter.compile("real_sample_{}".format(batch_idx * params.batch_size+i), coors=coor, intensity=pts_input[i][:,3],
